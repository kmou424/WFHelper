package moe.kmou424.WorldFlipper.Helper.Constants;

public class SharedPreferencesConfigs {
    public final static String BOSS_LEVEL_PRIMARY = "boss_level_primary";
    public final static String BOSS_LEVEL_MIDDLE = "boss_level_middle";
    public final static String BOSS_LEVEL_HIGH = "boss_level_high";
    public final static String BOSS_LEVEL_HIGH_PLUS = "boss_level_high_plus";
    public final static String BOSS_LEVEL_SUPER = "boss_level_super";
    public final static String WAIT_OTHERS_READY_CHECKBOX = "wait_others_ready_checkbox";
    public final static String BELL_TRACKER_SWITCH = "bell_tracker_switch";
}
